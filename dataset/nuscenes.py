...
class NuscenesLoader(Dataset):
    def __init__(self, root_dir, nusc=None, datum_names=CAMERA_NAMES, min_num_lidar_points=5, min_box_visibility=0.2, mode='train'):
        ...
        print("dateset length: ", len(self.samples))
...
class BEVNuscenesLoader(NuscenesLoader):
    ...
    def __getitem__(self, index):
        ...
        for cam in self.cams:
            ...
            img = img.resize((self.width, raw_H//2), resample=Image.NEAREST)
            img = img.crop((0, 1, self.width, self.height))
            intrin[0:2, :] *= (self.width / raw_W) #1600x900 -> 800x450
            intrin[1, 2] -= 1
